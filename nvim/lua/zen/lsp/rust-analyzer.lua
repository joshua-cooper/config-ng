...
local function notify_server_error(err)
    vim.notify(
        string.format("[rust-analyzer] error %d: %s", err.code, err.message),
...
local function notify_cargo_metadata_error(stderr)
    vim.schedule(function()
        vim.notify_once(
            string.format("Got an error from `cargo metadata`:\n\n%s", stderr),
            vim.log.levels.ERROR
        )
    end)
end
...
function M.rebuild_proc_macros()
    broadcast_request("rust-analyzer/rebuildProcMacros", "Rebuilt proc macros")
end
...
function M.root_dir(bufnr, on_dir)
    ...
    local opts = {
        ...
        cwd = crate_dir,
        env = {
            -- Allow cargo to parse manifests with unstable features.
            RUSTC_BOOTSTRAP = "1",
        },
    }
...
function M.reuse_client(client, config)
    ...
    local should_add_to_workspace =
        not has_workspace_folder(client.workspace_folders, config.root_dir)
